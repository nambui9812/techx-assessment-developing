use axum::{
    extract::{Path, State}, http::{HeaderMap, StatusCode}, routing::{delete, get}, Json, Router
};
...
async fn create_user(
    ...
    let user = sqlx::query_as::<_, UserRow>("INSERT INTO users(username) VALUES ($1)")
        .bind(body.username)
        .fetch_one(&pool)
        .await
        .map_err(|e| {
...
async fn create_workspace(
    ...
    let workspace = sqlx::query_as::<_, WorkspaceRow>("INSERT INTO workspaces(name, description, status, owner_id) VALUES ($1, $2, $3, $4)")
        .bind(body.name)
...
async fn update_workspace_status(
    ...
    let workspace = sqlx::query_as::<_, WorkspaceRow>("UPDATE workspaces SET status = $1 WHERE id = $2")
        .bind(body.status)
        .bind(workspace_id)
        .bind(Status::Active)
        .bind(onwer_id)
